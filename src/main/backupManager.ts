...
export class BackupManager {
  ...
  async createBackup(serverPath: string): Promise<string> {
  ...
  async listBackups(serverPath: string): Promise<string[]> {
    const backupDir = path.join(serverPath, this.backupDir);
  ...
  async restoreBackup(serverPath: string, backupName: string): Promise<void> {
    const configDir = await this.findConfigDir(serverPath);
    const backupDir = path.join(serverPath, this.backupDir);
    const backupPath = path.join(backupDir, backupName);
    ...
    try {
      ...
      const configFiles = backupFiles.filter(file => 
        file.endsWith('.ini') || 
        file.endsWith('.json') ||
        file.endsWith('.bak')
      );
  ...
  async deleteBackup(serverPath: string, backupName: string): Promise<void> {
    const backupDir = path.join(serverPath, this.backupDir);
    const backupPath = path.join(backupDir, backupName);
  ...
  async getBackupInfo(serverPath: string, backupName: string): Promise<any> {
    const backupDir = path.join(serverPath, this.backupDir);
    const backupPath = path.join(backupDir, backupName);
  ...
  async cleanupOldBackups(serverPath: string, maxBackups: number = 10): Promise<void> {
    try {
  ...
  async createBackupWithOptions(options: { name: string; description?: string; serverPath: string }): Promise<any> {
    try {
